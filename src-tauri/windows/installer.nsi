...
Section Uninstall
  ...
  ; Delete PulseDock app data for a full cleanup when not updating.
  ${If} $UpdateMode <> 1
    ...
    RmDir /r "$APPDATA\${MAINBINARYNAME}"
    RmDir /r "$LOCALAPPDATA\${MAINBINARYNAME}"
    RmDir /r "$LOCALAPPDATA\${PRODUCTNAME}.exe.WebView2"
    RmDir /r "$LOCALAPPDATA\${MAINBINARYNAME}.exe.WebView2"
  ${EndIf}
