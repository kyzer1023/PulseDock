...
impl AppState {
    fn new() -> Self {
        ...
        let runtime = if let Some(output_path) = smoke_output_path {
            AppRuntime::Smoke(Mutex::new(SmokeRuntime {
                output_path,
...
enum AppRuntime {
    ...
    Smoke(Mutex<SmokeRuntime>),
}

impl AppRuntime {
    async fn get_snapshot(&self) -> DashboardSnapshot {
        match self {
            Self::Normal(runtime) => runtime.get_snapshot().await,
            Self::Smoke(runtime) => runtime.lock().await.snapshot.clone(),
        }
    }

    async fn refresh(&self, app: &AppHandle) -> Result<DashboardSnapshot, String> {
        match self {
...
fn spawn_refresh(app: AppHandle) {
    tauri::async_runtime::spawn(async move {
        ...
        if let Err(error) = state.runtime.refresh(&app).await {
...
async fn get_dashboard(state: State<'_, AppState>) -> Result<DashboardSnapshot, String> {
    Ok(state.runtime.get_snapshot().await)
}
...
async fn refresh_dashboard(
    ...
    state.runtime.refresh(&app).await
}
...
async fn set_dashboard_usage_range(
    ...
    state.runtime.set_usage_range(&app, parsed_range).await
}
...
async fn write_smoke_result(
    ...
    let path = match &state.runtime {
        AppRuntime::Smoke(runtime) => runtime.lock().await.output_path.clone(),
        AppRuntime::Normal(_) => String::new(),
    };
    if path.is_empty() {
